import type { SegmentDef, MessageDef, ElementDef, SubElementDef, Requirement, LengthType } from './types'
...
function mhdSegment(expectedType: string, typeDesc: string): SegmentDef {
  return seg('MHD', 'MESSAGE HEADER', 'One mandatory occurrence per message', 'M', 'Once', [
    elem('MSRF', 'Message reference', 'Consecutive count of messages within the file: start at 1 and increment by 1 for each new message header.', 'M', '=', [
...
const mtrSegment: SegmentDef = seg('MTR', 'MESSAGE TRAILER', 'One mandatory occurrence per message', 'M', 'Once', [
  elem('NOSG', 'Number of segments in message', 'Control count of segments comprising the message, including MHD and MTR.', 'M', '=', [
...
const typSegment: SegmentDef = seg('TYP', 'TRANSACTION TYPE DETAILS', 'Specifies the type of invoice. Invoice types must not be mixed within a single file.', 'M', 'Once', [
  elem('TCDE', 'Transaction code', 'Code List 2: 0700 = Original invoice (VAT invoice), 0709 = Copy invoice (not for VAT purposes)', 'M', '=', [
    sub(0, 'Transaction code', '0700 = Original invoice, 0709 = Copy invoice', 'M', 'F', '9(4)'),
  ]),
...
const sdtSegment: SegmentDef = seg('SDT', 'SUPPLIER DETAILS', 'Identifies the supplier/sender of the invoice file. Either GLN or alternative code (or both) must be sent.', 'M', 'Once', [
  elem('SIDN', "Supplier's identity", "Coded identity of the supplier, preferably as an EAN location number (GLN).", 'M', '=', [
    sub(0, "Supplier's EAN location number (GLN)", 'EAN location number identifying the supplier.', 'C', 'F', '9(13)'),
    sub(1, "Supplier's identity allocated by customer", 'Alternative supplier code agreed between trading partners.', 'C', 'V', 'X(17)'),
  ]),
  elem('SNAM', "Supplier's name", "Supplier's legal name as printed on invoices. Required by HMRC unless code method approved.", 'C', '+', [
    sub(0, "Supplier's name", 'Legal name', 'C', 'V', 'X(40)'),
  ]),
  elem('SADD', "Supplier's address", "Max 5 lines. Required by HMRC unless code method approved.", 'C', '+', [
  ...
  elem('VATN', "Supplier's VAT registration no", "Mandatory even if the whole invoice is zero-rated.", 'C', '+', [
    sub(0, 'VAT number – numeric', "UK VAT number allocated by HMRC.", 'C', 'F', '9(9)'),
    sub(1, 'VAT number – alphanumeric', "Non-UK or with country code (e.g., GB987654321).", 'C', 'V', 'X(17)'),
  ]),
])

// ─── CDT ────────────────────────────────────────────────────────────────────

const cdtSegment: SegmentDef = seg('CDT', 'CUSTOMER DETAILS', 'Identifies the customer to whom the invoice file is addressed.', 'M', 'Once', [
  elem('CIDN', "Customer's identity", '', 'M', '=', [
    sub(0, "Customer's EAN location no (GLN)", 'EAN location number. Use strongly recommended.', 'C', 'F', '9(13)'),
    sub(1, "Customer's identity allocated by supplier", "Alternative code, may be customer's SAN.", 'C', 'V', 'X(17)'),
  ]),
  ...
  elem('VATR', "Customer's VAT registration no", "Required only for supply to customer in different EU country.", 'C', '+', [
    sub(0, 'VAT registration no – numeric', 'UK VAT number.', 'C', 'F', '9(9)'),
    sub(1, 'VAT registration no – alphanumeric', 'Non-UK or with country code.', 'C', 'V', 'X(17)'),
  ]),
])

// ─── DNA (file header level) ────────────────────────────────────────────────

const dnaFileHeaderSegment: SegmentDef = seg('DNA', 'DATA NARRATIVE', 'Carries BIC message/code list version numbers or currency code. Two occurrences recommended.', 'C', 'Repeat as necessary', [
  elem('SEQA', 'First level sequence number', 'Starts at 1, incremented by 1 for each repeat.', 'M', '=', [
    sub(0, 'Sequence number', '', 'M', 'V', '9(10)'),
  ]),
  elem('DNAC', 'Data narrative code', 'Code table reference. 206 = BIC message version, 207 = BIC code list version.', 'C', '+', [
    sub(0, 'Code table number', '206 = message version (T02), 207 = code list version', 'C', 'V', '9(4)'),
...
const filSegment: SegmentDef = seg('FIL', 'FILE DETAILS', 'File sequence number, version number and date.', 'M', 'Once', [
  elem('FLGN', 'File generation number', 'Sequential for each successive Invoice file exchanged between trading partners.', 'M', '=', [
    sub(0, 'File generation number', '', 'M', 'V', '9(4)'),
  ]),
  elem('FLVN', 'File version number', "Retransmission indicator. Original is always '1'. Incremented for each retransmission.", 'M', '+', [
...
const cloSegment: SegmentDef = seg('CLO', "CUSTOMER'S LOCATION", "Identifies delivery location for items invoiced. Must be included even if same as CDT in file header.", 'M', 'Once', [
  elem('CLOC', "Customer's location", 'One of three customer references must be present.', 'M', '=', [
    sub(0, "Customer's EAN location number (GLN)", '', 'C', 'F', '9(13)'),
    sub(1, "Customer's own location code", 'Branch or department code', 'C', 'V', 'X(17)'),
    sub(2, "Supplier's identification of customer's location", "Supplier's reference / SAN of delivery location", 'C', 'V', 'X(17)'),
  ]),
...
const irfSegment: SegmentDef = seg('IRF', 'INVOICE REFERENCES', 'Date of Invoice and Tax-point Date.', 'M', 'Once', [
...
const pytSegment: SegmentDef = seg('PYT', 'SETTLEMENT TERMS', 'Settlement terms for the invoice. BIC recommends days-based or fixed-date approach.', 'C', 'Repeat as necessary', [
  ...
  elem('PAYY', 'Settlement terms', 'Number of days until payment is due.', 'C', '+', [
    sub(0, 'Number of days', 'Calendar days after reference date (date of invoice unless otherwise agreed)', 'M', 'V', '9(3)'),
...
const dnaInvoiceSegment: SegmentDef = seg('DNA', 'DATA NARRATIVE', 'Coded or free text at invoice level. RTEX codes: 978 (Cancelled invoice number), 979 (Supplier internal code), 984 (Cancelled credit note number).', 'C', 'Repeat as necessary', [
...
const oddSegment: SegmentDef = seg('ODD', 'ORDER AND DELIVERY REFERENCES', 'Introduces a group of invoice lines from a single order/delivery. Repeated for each order.', 'M', 'Repeat for each order', [
  elem('SEQA', 'First level sequence number', 'Starts at 1, incremented by 1.', 'M', '=', [
    sub(0, 'Sequence number', '', 'M', 'V', '9(10)'),
  ]),
  elem('ORNO', 'Order number and date', '', 'M', '+', [
    ...
    sub(2, 'Date order placed by customer', 'YYMMDD. Not required if order number sent.', 'C', 'F', '9(6)'),
    sub(3, 'Date order received by supplier', 'YYMMDD', 'C', 'F', '9(6)'),
  ]),
  elem('DELN', 'Delivery note details', 'Either/both delivery note number and/or despatch date must be sent.', 'M', '+', [
    sub(0, 'Delivery note number', 'Allocated by the supplier.', 'C', 'V', 'X(17)'),
    sub(1, 'Date of document', 'YYMMDD. Date of despatch.', 'C', 'F', '9(6)'),
  ]),
  elem('NODU', 'Number of delivery or uplift units', 'Estimated number of packages (only if Invoice is delivery notification).', 'C', '+', [
  ...
  elem('SCRF', 'Specification/Contract references', 'Special deal or promotion reference.', 'C', '+', [
    sub(0, 'Specification number', 'Do not use', 'C', 'V', 'X(17)'),
    sub(1, 'Contract number', 'Special deal or promotion. Use "/" as delimiter.', 'C', 'V', 'X(17)'),
  ]),
])

// ─── ILD ────────────────────────────────────────────────────────────────────

const ildSegment: SegmentDef = seg('ILD', 'INVOICE LINE DETAILS', 'One occurrence per invoice line. For sundry charges, refer to section 7.', 'M', 'Repeat for each line item', [
  elem('SEQA', 'First level sequence number', 'Same value as corresponding ODD segment.', 'M', '=', [
    sub(0, 'Sequence number', 'Matches ODD/SEQA', 'M', 'V', '9(10)'),
  ]),
  elem('SEQB', 'Second level sequence number', 'Invoice line number. Starts at 1, incremented by 1.', 'M', '+', [
    sub(0, 'Line number', '', 'M', 'V', '9(10)'),
  ]),
  elem('SPRO', "Supplier's product number", 'Product identification. ISBN-13/EAN-13, or charge code from Code List 205 for sundry charges.', 'M', '+', [
    sub(0, 'EAN-13 article number', '"Bookland" EAN-13 or ISBN-13', 'C', 'F', '9(13)'),
    sub(1, "Supplier's code for traded unit", 'ISBN-10 (deprecated) or charge code from Code List 205', 'C', 'V', 'X(30)'),
  ...
  elem('AUCT', 'Unit cost price (excl. VAT)', 'Net unit cost after discounts, before VAT. Always 4 decimal places (e.g., £12.99 = 129900).', 'M', '+', [
    sub(0, 'Cost price', 'In pounds, 4 implied decimal places', 'M', 'V', '9(10)V9(4)'),
    sub(1, 'Measure indicator', 'Do not use', 'C', 'V', 'X(6)'),
  ]),
  elem('LEXC', 'Extended line cost (excl. VAT)', 'Total line cost after discounts, before VAT. 4 decimal places.', 'M', '+', [
    sub(0, 'Extended line cost', 'In pounds, 4 implied decimal places', 'M', 'V', '9(10)V9(4)'),
  ]),
  elem('VATC', 'VAT Rate category code', 'Code List 12: S=Standard, Z=Zero-rated, A=Mixed, O=Outside scope', 'M', '+', [
  ...
  elem('MIXI', 'Mixed VAT Rate product indicator', 'Only for mixed-rate items. 0=product as whole, 1=zero-rated component, 2=standard-rate component.', 'C', '+', [
  ...
  elem('TDES', 'Traded unit description', 'Use for substituted items or description-only orders.', 'C', '+', [
    sub(0, 'Description line 1', 'Author', 'C', 'V', 'X(40)'),
    sub(1, 'Description line 2', 'Title', 'C', 'V', 'X(40)'),
  ]),
  elem('MSPR', 'Selling on price', "Manufacturer's recommended selling price including VAT. Required in book trade.", 'C', '+', [
    sub(0, "Manufacturer's recommended selling price", 'In pounds, 4 decimal places', 'C', 'V', '9(10)V9(4)'),
  ...
  elem('BUCT', 'Unit cost price (excl. VAT) before discount', 'Required in book trade. 4 decimal places.', 'C', '+', [
    sub(0, 'Unit cost before discount', 'In pounds, 4 decimal places', 'C', 'V', '9(10)V9(4)'),
  ]),
  elem('DSCV', 'Discount value', 'Line discount value in pounds. Required. 4 decimal places.', 'C', '+', [
  ...
  elem('PIND', 'Special Price indicator', 'Code List 5. F = free of charge (all monetary values carry zeros).', 'C', '+', [
    sub(0, 'Price indicator code', '', 'C', 'V', 'X(4)'),
  ]),
  elem('IGPI', 'Item Group identifier', 'Code List 10. I = line-level charge, G = invoice-level charge.', 'C', '+', [
    sub(0, 'Group identifier', 'I or G', 'C', 'V', 'X(4)'),
  ]),
  elem('CSDI', 'Cash settlement discount identifier', 'Do not use.', 'C', '+', [
    sub(0, 'Identifier', '', 'C', 'F', 'X(1)'),
  ]),
  elem('TSUP', 'VAT – Type of supply', 'Code List 14. Omitted for normal sale (code A).', 'C', '+', [
...
const dncSegment: SegmentDef = seg('DNC', 'DATA NARRATIVE', 'Qualifies the invoice line. One DNC per ILD (for mixed-rate: one DNC after 3rd ILD).', 'C', 'One per ILD group', [
  ...
  elem('SEQC', 'Third level sequence number', 'Starts at 1, incremented by 1 for each repeat.', 'M', '+', [
    sub(0, 'Sub-sequence', '', 'M', 'V', '9(10)'),
  ]),
  elem('DNAC', 'Data narrative code', 'Only BIC list 203 (Order qualifier: BIC, FMS, SLR, SSF).', 'C', '+', [
    sub(0, 'Code table number', '', 'C', 'V', '9(4)'),
    sub(1, 'Code value', '', 'C', 'V', 'X(3)'),
  ]),
  elem('RTEX', 'Registered text', 'Valid codes: 073 (currency), 082 (order line ref), 314 (binder pack qty), 971 (component items), 980 (SOR number), 982 (HMRC commodity code).', 'C', '+', [
    sub(0, '1st registered application code', '', 'C', 'V', 'X(3)'),
    sub(1, 'Application text', '', 'C', 'V', 'X(40)'),
    sub(2, '2nd registered application code', '', 'C', 'V', 'X(3)'),
    sub(3, 'Application text', '', 'C', 'V', 'X(40)'),
...
const stlSegment: SegmentDef = seg('STL', 'VAT RATE INVOICE SUB-TRAILER', 'Repeated for each real VAT rate in the invoice. Not for A (mixed rate).', 'M', 'Repeat for each VAT rate', [
  elem('SEQA', 'First level sequence number', 'Starts at 1.', 'M', '=', [
    sub(0, 'Sequence number', '', 'M', 'V', '9(10)'),
  ]),
  ...
  elem('NRIL', 'Number of item lines', 'Count of ILD segments with this VAT code (including mixed-rate components).', 'M', '+', [
  ...
  elem('QYDA', 'Discount amount for invoice quantity', 'Not used in book trade practice.', 'C', '+', [
    sub(0, 'Quantity discount', '', 'C', 'V', '9(10)V9(2)'),
  ]),
  elem('VLDA', 'Discount amount for invoice value', 'Not used in book trade practice.', 'C', '+', [
  ...
  elem('EVLA', 'Extended sub-total amount (before settlement discount)', 'EVLA = LVLA – QYDA – VLDA + SURA – SSUB (or = LVLA if others unused)', 'M', '+', [
    sub(0, 'Extended sub-total', 'In pounds, 2 decimal places', 'M', 'V', '9(10)V9(2)'),
  ]),
  elem('SEDA', 'Sub-total settlement discount amount', 'Only if payment terms include settlement discount.', 'C', '+', [
    sub(0, 'Settlement discount', '', 'C', 'V', '9(10)V9(2)'),
  ]),
  elem('ASDA', 'Extended sub-total amount (after settlement discount)', 'ASDA = EVLA – SEDA (or = EVLA if no settlement discount)', 'M', '+', [
  ...
  elem('APSE', 'Payable sub-total (before settlement discount)', 'APSE = EVLA + VATA. Only if settlement discount applies.', 'C', '+', [
...
const tlrSegment: SegmentDef = seg('TLR', 'INVOICE TRAILER', 'Totals for the invoice message as a whole.', 'M', 'Once', [
  ...
  elem('QYDT', 'Total discount amount for invoice quantity', 'Not used in book trade.', 'C', '+', [
  ...
  elem('TPSE', 'Total payable (before settlement discount)', 'Σ APSE. Only if settlement discount applies.', 'C', '+', [
    sub(0, 'Total payable before settlement', '', 'C', 'V', '9(10)V9(2)'),
  ]),
  elem('TPSI', 'Total payable (after settlement discount)', 'Σ APSI. Final total payable including VAT.', 'M', '+', [
...
const vrsSegment: SegmentDef = seg('VRS', 'VAT RATE SUMMARY', 'Repeated for each real VAT rate in the file. A (mixed) is not a real rate.', 'M', 'Repeat for each VAT rate', [
  elem('SEQA', 'First level sequence number', 'Starts at 1.', 'M', '=', [
    sub(0, 'Sequence number', '', 'M', 'V', '9(10)'),
  ]),
  ...
  elem('VSDE', 'File sub-total amount (before settlement discount)', 'Σ EVLA across all STL with this VAT code', 'M', '+', [
    sub(0, 'File sub-total before settlement', 'In pounds, 2 decimal places', 'M', 'V', '9(10)V9(2)'),
  ]),
  elem('VSDI', 'File sub-total amount (after settlement discount)', 'Σ ASDA across all STL with this VAT code', 'M', '+', [
    sub(0, 'File sub-total after settlement', 'In pounds, 2 decimal places', 'M', 'V', '9(10)V9(2)'),
  ]),
  ...
  elem('VPSE', 'File sub-total payable (before settlement discount)', 'Σ APSE. Only if settlement discount applies.', 'C', '+', [
    sub(0, 'File payable before settlement', '', 'C', 'V', '9(10)V9(2)'),
  ]),
  elem('VPSI', 'File sub-total payable (after settlement discount)', 'Σ APSI across all STL with this VAT code', 'M', '+', [
    sub(0, 'File payable after settlement', 'In pounds, 2 decimal places', 'M', 'V', '9(10)V9(2)'),
  ]),
])

// ─── TOT ────────────────────────────────────────────────────────────────────

const totSegment: SegmentDef = seg('TOT', 'FILE TOTALS', 'Mandatory control totals for the invoice file.', 'M', 'Once', [
  elem('FASE', 'File total amount (before VAT and settlement discount)', 'Σ VSDE', 'M', '=', [
    sub(0, 'File total before VAT/settlement', 'In pounds, 2 decimal places', 'M', 'V', '9(10)V9(2)'),
  ]),
  ...
  elem('FPSE', 'File total payable (after VAT, before settlement discount)', 'Σ VPSE. Only if settlement discount applies.', 'C', '+', [
    sub(0, 'File payable before settlement', '', 'C', 'V', '9(10)V9(2)'),
  ]),
  ...
  elem('FTNI', 'File total number of invoice messages', 'Total number of Invoice messages in the file.', 'M', '+', [
...
export const endSegmentDef: SegmentDef = seg('END', 'END OF TRANSMISSION', 'Transmission envelope trailer.', 'M', 'Once', [
...
export const invfilMessageDef: MessageDef = {
  ...
  description: 'One occurrence at the start of the file. Contains supplier/customer details and file metadata.',
  segments: [
...
export const vattlrMessageDef: MessageDef = {
  ...
  description: 'One occurrence after all INVOIC messages. Summarises VAT by rate for the whole file.',
  segments: [
    mhdSegment('VATTLR', 'File VAT trailer'),
    vrsSegment,
    mtrSegment,
  ],
}
...
export const invtlrMessageDef: MessageDef = {
  ...
  segments: [
    mhdSegment('INVTLR', 'Invoice file trailer'),
    totSegment,
    mtrSegment,
  ],
}
