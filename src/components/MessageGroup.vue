...
const hasWarnings = computed(() => messageIssues.value.some(i => i.severity === 'warning') && !hasErrors.value)
</script>
...
<template>
  ...
  >
    ...
    <div v-if="messageIssues.length > 0" class="select-none px-3 py-1.5 border-b bg-red-50/50 dark:bg-red-950/20 text-xs space-y-0.5">
      <div
        v-for="(iss, issIdx) in messageIssues"
        :key="issIdx"
    ...
    <div class="py-0.5">
      <SegmentLine
        v-for="(seg, segIdx) in message.segments"
        :key="segIdx"
        :segment="seg"
      />
    </div>
