  ...
  >
    ...
    <FieldTooltip>
      ...
      <template #content>
        <div class="px-3 py-2 space-y-1.5 min-w-[260px] max-w-md">
          ...
          <div v-if="segment.def?.description" class="text-xs text-muted-foreground">
            {{ segment.def.description }}
          </div>
          <div class="flex gap-2 text-xs mt-1">
            <span
              class="inline-flex items-center rounded-full px-2 py-0.5 font-medium"
              :class="
                segment.def?.requirement === 'M'
            ...
            <span v-if="segment.def?.repeat" class="text-muted-foreground">{{
              segment.def.repeat
            }}</span>
          </div>
          ...
          <div v-if="segment.def" class="border-t pt-1.5 mt-1.5 space-y-0.5">
            ...
            >
              <span class="font-mono text-primary/70 shrink-0 w-10"
                >{{ segment.tag }}{{ String(eIdx + 1).padStart(2, '0') }}</span
              >
              ...
              >
              <span v-else-if="elDef.requirement === 'M'" class="ml-auto text-[10px] text-red-500"
                >empty</span
              >
            </div>
          </div>
    ...
    <template v-for="(el, elIdx) in segment.elements" :key="elIdx">
      ...
      <span
        ...
        :class="{
          'bg-primary/5 outline outline-1 outline-primary/20':
            hoveredElIdx === elIdx && el.subElements.length > 1,
        }"
      >
            ...
            <span
              ...
              :class="{
                ...
                'text-foreground font-medium':
                  sub.def?.requirement === 'M' && !subHasIssue(sub) && sub.raw !== '',
                'text-zinc-500 dark:text-zinc-400':
                  sub.def?.requirement !== 'M' && !subHasIssue(sub) && sub.raw !== '',
                'bg-red-100/50 dark:bg-red-900/20 border-b border-dashed border-red-400':
                  sub.raw === '' && sub.def?.requirement === 'M' && isMandatoryElementEmpty(elIdx),
                'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400':
                  subHasIssue(sub) && subIssueSeverity(sub) === 'error',
                'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400':
                  subHasIssue(sub) && subIssueSeverity(sub) === 'warning',
                'bg-sky-50 text-sky-700 dark:bg-sky-900/20 dark:text-sky-400':
                  subHasIssue(sub) && subIssueSeverity(sub) === 'info',
                'hover:bg-sky-50 dark:hover:bg-sky-900/20':
                  !subHasIssue(sub) && !isHighlighted(sub.raw),
                'ring-2 ring-primary/40 bg-primary/10': sub.raw !== '' && isHighlighted(sub.raw),
              }"
              @mouseenter="onSubEnter(sub, elIdx)"
              @mouseleave="onSubLeave()"
              ><template v-if="isDecimalFormatted(sub)"
                ><span class="tabular-nums">{{ getDisplayValue(sub).split('.')[0] }}</span
                ><span class="select-none text-emerald-600 dark:text-emerald-400 font-bold">.</span
            ...
            <template #content>
              <div class="px-3 py-2 space-y-1.5 min-w-[200px]">
                ...
                <div class="flex items-baseline gap-2">
                  <span
                    class="font-mono text-xs text-primary/80 bg-primary/10 rounded px-1.5 py-0.5 select-none"
                    >{{ getFieldRef(el) }}</span
                  >
                  <span class="font-semibold text-sm">{{
                    sub.def?.name ?? `Field ${sub.index + 1}`
                  }}</span>
                </div>
                ...
                <div
                  v-if="sub.def?.format"
                  class="text-[10px] text-muted-foreground/70 font-mono mt-0.5"
                >
                  TRADACOMS: {{ sub.def.format.notation
    ...
    <span
      class="ml-4 select-none text-xs text-muted-foreground/60 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap"
